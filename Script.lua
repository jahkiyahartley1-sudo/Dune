...
-- // Server-side executor with require() and full SS support
local function execServer(code)
	-- Wrap code to support require() and common SS globals
	local wrapped = string.format([[
		local _ENV = getfenv and getfenv() or _G
		-- Provide safe require wrapper
		local _require = require
		require = function(id)
			local ok, result = pcall(_require, id)
			if ok then return result end
			-- Try numeric asset ID fallback
			if type(id) == "string" and tonumber(id) then
				ok, result = pcall(_require, tonumber(id))
				if ok then return result end
			end
	...
	local success, err = pcall(function()
		...
		if env == "synapse" then
			syn.run_on_server(code) -- Synapse SS: send raw, server handles env

		elseif env == "krnl" then
			-- KRNL server-side via its own API
			if krnl_request then
				krnl_request({ Type = "ServerExec", Code = code })
			else
				loadstring(code)()
			end
		...
		elseif env == "genv_se" then
			...
			-- Fallback: local loadstring with wrapped env
			local fn, loadErr = loadstring(wrapped)
			if not fn then
...
local function fetchURL(url)
	local ok, result = pcall(function()
		...
		elseif request then
			...
			-- Try Roblox HttpService (only works in some executors)
...
MakeControlBtn(-40, Color3.fromRGB(255, 189, 46), "-", function()
	if Main.Size.Y.Offset < 100 then
		Tween(Main, {Size = UDim2.new(0, 700, 0, 500)}, 0.3)
	else
		Tween(Main, {Size = UDim2.new(0, 700, 0, 38)}, 0.3)
	end
end)
MakeControlBtn(-60, Color3.fromRGB(40, 205, 65), "+", function()
	if Main.Size.X.Offset > 720 then
		...
		Tween(Main, {Size = UDim2.new(0, 950, 0, 620)}, 0.3)
	end
end)
...
Editor.PlaceholderText = "-- Paste server-side script here\n-- Supports: require(), loadstring(), remote scripts\n-- Ctrl+Enter = Execute  |  M = Command Bar"
...
-- // URL Input Bar (for remote loading)
...
-- NOTE: positioning FetchExecBtn just to the left of FetchBtn
...
FetchBtn.MouseButton1Click:Connect(function()
	...
	SetStatus("Fetching " .. url:sub(1, 50) .. "...", WARNING)
	local ok, result = fetchURL(url)
	if ok then
		Editor.Text = result
		if activeTab then tabs[activeTab].content = result end
		SetStatus("Loaded from URL — " .. #result .. " bytes", SUCCESS)
	else
		SetStatus("Fetch failed: " .. tostring(result), ERROR_C)
	end
end)
...
FetchExecBtn.MouseButton1Click:Connect(function()
	local url = URLInput.Text
	if url == "" then SetStatus("Enter a URL first!", WARNING) return end
	SetStatus("Fetching & executing " .. url:sub(1, 40) .. "...", WARNING)
	local ok, result = fetchURL(url)
	...
	if ok2 then
		SetStatus("Remote script executed! (" .. #result .. " bytes)", SUCCESS)
	else
		SetStatus("Exec error: " .. tostring(err2), ERROR_C)
	end
end)
...
MakeButton("📋 COPY", UDim2.new(0, 214, 0, 52), 75, BG_PANEL, function()
	if setclipboard then
		...
		SetStatus("setclipboard not supported", WARNING)
	end
end)
...
MakeButton("💾 SAVE", UDim2.new(0, 297, 0, 52), 75, BG_PANEL, function()
	...
	if writefile then
		pcall(writefile, "executor_script.lua", Editor.Text)
		SetStatus("Saved to executor_script.lua", SUCCESS)
	end
end)
...
MakeButton("📂 LOAD", UDim2.new(0, 380, 0, 52), 75, BG_PANEL, function()
	pcall(function()
		if readfile then
			local content = readfile("executor_script.lua")
			if content then
				Editor.Text = content
				SetStatus("Loaded executor_script.lua", SUCCESS)
			end
		else
			SetStatus("readfile not supported", WARNING)
		end
	end)
end)

MakeButton("🔁 RELOAD", UDim2.new(0, 463, 0, 52), 80, Color3.fromRGB(40, 40, 20), function()
	-- Re-fetch the GitHub script and reload this executor
	local url = "https://raw.githubusercontent.com/PineappleJuiceFlavour/Dune/refs/heads/main/Script.lua"
	SetStatus("Reloading executor from GitHub...", WARNING)
	local ok, result = fetchURL(url)
	if ok then
		...
		if fn then
			...
			SetStatus("Reload compile error: " .. tostring(err), ERROR_C)
		end
	else
		SetStatus("Reload fetch failed: " .. tostring(result), ERROR_C)
	end
end)
...
local function SwitchTab(id)
	if activeTab and tabs[activeTab] then
		...
		if old.button then
			Tween(old.button, {BackgroundColor3 = BG_LIGHT}, 0.15)
			old.label.TextColor3 = TEXT_DIM
		end
	end
	activeTab = id
	if tabs[id] then
		...
		if tabs[id].button then
			...
			tabs[id].label.TextColor3 = Color3.fromRGB(255, 255, 255)
		end
...
local function CreateTab(name)
	if tabCount >= MAX_TABS then SetStatus("Max " .. MAX_TABS .. " tabs!", WARNING) return end
	tabCount += 1
	local id = tabCount
...
-- Built-in commands
local commands = {
	clear   = function() Editor.Text = "" SetStatus("Cleared via command", TEXT_DIM) end,
	exec    = function(args)
	...
	require = function(args)
		-- Run a require() call server-side
		local id = args[1]
		if not id then SetStatus("Usage: require <assetId>", WARNING) return end
		local ok, err = execServer("require(" .. id .. ")")
		SetStatus(ok and "require("..id..") called!" or "Error: "..tostring(err), ok and SUCCESS or ERROR_C)
	end,
	fetch   = function(args)
		local url = args[1]
		if not url then SetStatus("Usage: fetch <url>", WARNING) return end
		SetStatus("Fetching...", WARNING)
		local ok, result = fetchURL(url)
		if ok then Editor.Text = result SetStatus("Fetched "..#result.." bytes", SUCCESS)
		else SetStatus("Fetch failed: "..tostring(result), ERROR_C) end
	end,
	fetchrun = function(args)
		local url = args[1]
		if not url then SetStatus("Usage: fetchrun <url>", WARNING) return end
		SetStatus("Fetching & executing...", WARNING)
		local ok, result = fetchURL(url)
		if not ok then SetStatus("Fetch failed: "..tostring(result), ERROR_C) return end
		Editor.Text = result
		local ok2, err2 = execServer(result)
		SetStatus(ok2 and "Remote executed!" or "Error: "..tostring(err2), ok2 and SUCCESS or ERROR_C)
	end,
	newtab  = function(args) CreateTab(args[1]) end,
	print   = function(args)
		local ok, err = execServer("print("..table.concat(args," ")..")")
		SetStatus(ok and "print() sent" or tostring(err), ok and SUCCESS or WARNING)
	end,
	env     = function() SetStatus("Executor: " .. getExecEnv(), ACCENT) end,
	help    = function()
		SetStatus("clear | exec <code> | require <id> | fetch <url> | fetchrun <url> | newtab | print | env", ACCENT)
	end,
}
...
SetStatus("v3.0 Ready  |  Executor: " .. getExecEnv():upper() .. "  |  [M] = Command Bar  |  Ctrl+Enter = Run", SUCCESS)
print("[ServerExecutor v3.0] Loaded! Press M for command bar.")
